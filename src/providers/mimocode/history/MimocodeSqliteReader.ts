...
export async function loadMimocodeSessionRows(
  databasePath: string,
  sessionId: string,
  dependencies: MimocodeSqliteReaderDependencies = {},
...
function resolveDependencies(
  dependencies: MimocodeSqliteReaderDependencies,
): Required<MimocodeSqliteReaderDependencies> {
  return {
    findNodeExecutable,
    requireSqliteModule,
...
function loadSessionRowsWithNodeProcess(
  ...
  const result = spawnSync(
    ...
    [
      '-e',
      MIMOCODE_SQLITE_CHILD_SCRIPT,
      databasePath,
      sessionId,
      MIMOCODE_MESSAGE_ROW_SQL,
      MIMOCODE_PART_ROW_SQL,
    ],
    {
      encoding: 'utf8',
      maxBuffer: MIMOCODE_SQLITE_QUERY_MAX_BUFFER,
      windowsHide: true,
    },
  );
